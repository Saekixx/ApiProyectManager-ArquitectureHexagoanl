...
public class ReopenTask {
    ...
    public ReopenTask(TaskRepository taskRepository, UserRepository userRepository) {
    ...
    public void execute(Integer taskId, Integer userId) {
        ...
        if (!isAdmin && !isLeader && !isAssigned) {
            throw new BusinessException("No tienes permisos para reabrir esta tarea. Debes ser Admin, Líder del proyecto o estar asignado a ella.");
        }
        // Cambiar el estado de la tarea a "En Progreso"
        task.getState().reabrir(task);
